...
**Files to modify:**
- `prisma/seed.mjs` — update recipe-card system prompt with RECIPE METADATA section, bump version to 3
- Create: `scripts/fix-recipe-card-prompt-v2.ts` — migration script to update DB prompt

## What to do

### 1. Update seed.mjs

Find the recipe-card prompt entry (around line 424-463). It has:
```javascript
slug: "recipe-card",
feature: "recipe-card",
...
Append the RECIPE METADATA section to the systemPrompt. The existing prompt ends with:
```
Если каких-то данных нет в запросе — используй пустой массив или пустую строку. Не придумывай.`,
```

Append after that (before the closing backtick):
```
...
Create `scripts/fix-recipe-card-prompt-v2.ts`:
...
```bash
npx tsx scripts/fix-recipe-card-prompt-v2.ts
```

Expected output: "Created recipe-card prompt version 3 with RECIPE METADATA section."

## Your Job

1. Read `prisma/seed.mjs` and find the recipe-card prompt entry
2. Update the seed: append RECIPE METADATA to systemPrompt, bump version to 3
3. Create `scripts/fix-recipe-card-prompt-v2.ts` with the above code
4. Run the migration script
5. Commit with: `git commit -m "feat(recipe-card): update AI prompt with RECIPE METADATA section"`

## Report Format

Write to `C:\Users\Roof\Documents\Телега\pastry-ai\.superpowers\sdd\task-6-report.md`:
- What you did
